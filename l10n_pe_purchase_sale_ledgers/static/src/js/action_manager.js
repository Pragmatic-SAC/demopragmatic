...
odoo.define('l10n_pe_purchase_sale_ledgers.ActionManagerExcelTxt', function (require) {
    "use strict";
    ...
    ActionManager.include({
         ...
         */
        _executeSPReportAccountReportDownloadAction: function (action) {
            var self = this;
            framework.blockUI();
            return new Promise(function (resolve, reject) {
                session.get_file({
        ...
        _handleAction: function (action, options) {
            console.log("action", action);
            if (action.report_type == 'xlsx_txt') {
                if (action.data.model == 'report.account.books') {
                    return this._executeSPReportAccountReportDownloadAction(action, options);
                }
            }
            return this._super.apply(this, arguments);
        },
